...
export class SessionDO extends DurableObject<Env> {
  ...
  private async startContainer(data: {
    ...
    try {
      ...
      // Auto-continue if there were pending questions/permissions from a previous session
      await this.autoContinueIfPending();
  ...
  /**
   * Auto-continue if there were pending questions/permissions from a previous session.
   * Sends "continue" to the container so the resumed SDK re-asks the pending question.
   */
  private async autoContinueIfPending(): Promise<void> {
    ...
    if (hasPending && this.containerWs?.readyState === WebSocket.OPEN) {
      console.log("[SessionDO] Auto-continuing for pending questions/permissions");
      this.containerWs.send(JSON.stringify({ type: "message", content: "continue" }));
